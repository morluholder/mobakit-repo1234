...
export class AddProductComponent implements OnInit {
  ...
  handleFileInput(files: FileList) {
    ...
    this.http.post('http://mobikart-alb-2069803266.us-east-1.elb.amazonaws.com/upload', formData).subscribe((res) => {
      console.log(res);
    });
  }
  ...
  AddProduct(regform: any) {
    ...
    console.log(data);
    this.http.post('http://mobikart-alb-2069803266.us-east-1.elb.amazonaws.com/process',data,{headers: new HttpHeaders({'Content-Type': 'application/json'})}).subscribe((res)=>{
      console.log(res);
    });*/
    const data = JSON.stringify(this.model);
